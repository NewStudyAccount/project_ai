...
首个系统脚手架落地后，应删除本「阶段说明」并同步核对全文是否仍符合现状。
...
- 消息队列：MQ（异步、重试、死信；**产品选型随变更提案锁定后回填本节**。选定前禁止业务绑死某一家客户端 API、禁止写消费者业务代码）
- 对象存储：MinIO（S3 兼容）
...
- CI/CD：Jenkins（流水线见 `docs/jenkins-pipeline-guide.md`，脚手架时若无则先建）
...
另一套锁客户端、另一套对象映射、另一套工具集）。具体坐标与版本以各系统父 `pom.xml` 的
`dependencyManagement` 为准，**不在本文件粘贴版本号**。

| 依赖 | 用途 | 落点 | 约定 |
|------|------|------|------|
| **MyBatis-Plus** | ORM、分页、逻辑删除、自动填充 | `{system}-framework` 装配；Entity/Mapper 在 `{domain}-service` | 禁止业务自建 `SqlSessionFactory` / 分页插件 / `MetaObjectHandler` |
| **Redis + Redisson** | 缓存、会话、分布式锁、幂等存储、`RRateLimiter` 限流 | `{system}-framework` 连接与序列化；业务注入使用 | 禁止自建 `RedisTemplate` / `RedissonClient`；缓存 key 见 6.9 |
| **Spring Cloud OpenFeign** | 服务间调用 | 契约在 `{domain}-api`；业务 `feign/` | 内部服务**唯一** HTTP 客户端；必须 Fallback；禁 RestTemplate/裸 HTTP 调内部服务 |
| **`@RateLimit` + Redisson `RRateLimiter`** | 接口/方法级限流 | `{system}-framework` | 见 6.11；禁止业务自建限流切面 |
| **`@Idempotent` + Redis** | 关键写防重/回放 | `{system}-framework` | 见 6.11；禁止业务自写 setnx、禁止用 `uk_` 做幂等 |
| **Lombok** | 样板消除 | 编译期，全模块可用 | 仅用于样板；CI 开启注解处理 |
| **Spring Boot Actuator** | 健康、存活、指标 | 可运行模块（`*-service` / `*-gateway`） | 必须暴露 `/actuator/health` |
| **MapStruct** | Entity ↔ DTO/VO/BO | 编译期，业务模块 | 映射接口**只**放 `converter/`；禁 `BeanUtils` 主路径拷贝 |
| **SpringDoc OpenAPI** | 接口文档 | `{system}-framework` 统一配置；业务只写注解 | 禁止各模块另起 Swagger 配置 |
| **Bean Validation** | 入参校验 | 全模块 | 见 6.4.4；失败走全局异常 |
| **Jackson** | JSON；Long→String | `{system}-framework` 统一 `ObjectMapper` | 禁止字段上散落 `@JsonSerialize`（见 6.4.7） |
| **SLF4J + Logback** | 日志 | 全模块 | 禁止 `System.out` / `printStackTrace` |
| **Micrometer Tracing** | 链路 TraceId/SpanId | `framework` 传播；可运行模块上报 | 禁止 Sleuth；见 6.11.1 |
| **消息客户端**（随 MQ 选定） | 异步、重试、死信 | `framework` 封装 | 禁止裸连 broker；未选型不写绑定产品的业务代码 |
| **MinIO / S3 客户端** | 文件上传下载 | `framework` 统一客户端 | 见 6.11；禁止各服务自建 SDK 配置 |
| **JWT**（库名待选定后回填） | 认证令牌 | `framework` 签发/校验 | Access 30 分钟、Refresh 7 天（见 6.7）；禁止业务自写 JWT 解析 |

**落点原则（与 5.4 / 6.4 一致）：**

- 需连接中间件或额外配置才能工作的依赖，**只**在 `{system}-framework` 封装一次。
- 纯编译期工具（Lombok、MapStruct）可随模块引入。
- `common` **禁止**依赖 MyBatis-Plus、Redis/Redisson、数据源、Actuator、MQ/MinIO 客户端、JWT 库、**Hutool**。
- 完整反模式清单见 **7.1**（本节不重复维护）。
...
| **独立熔断组件**（Resilience4j 等） | **暂不引入**；Feign/网关必须配置超时（建议连接 3s、读取 10s）；需要熔断时再开变更 |
...
- 各组件接入信息一律查 `docs/test-env.md`（脚手架时若无则先建）；本文件不写死具体值。
- 端口、网关前缀、环境差异以 `docs/test-env.md` 与 `application.yml` / `application-dev.yml` / `application-test.yml` 为准；生产敏感项以 Nacos / 密钥管理为准。

### 3.3 环境变量与本地配置

- 环境变量统一通过 `.env`（本地、gitignore）管理，禁止把配置散落硬编码在业务逻辑中。
- 命名：`SERVICE_NAME_ENV_KEY` 大写下划线。
...
- Java 包名前缀在**首个系统脚手架**时定为真实 group（当前文档示例中的 `com.example` 为占位，落地时替换并回写本节约定）。
...
| 其他 | 边缘跨域头、路径改写 | CORS 策略、用户身份注入/透传 | 参数校验、事务、业务规则 |
...
业务服务**不得**信任可伪造的客户端 `X-User-Id`（除非网关覆盖写入）；服务内以网关注入为准。
...
| 场景 | HTTP | `code` 语义 | 前端 |
|------|------|-------------|------|
| 未登录 / Token 无效 | 401 | 系统段（`1xxxx`）约定「未认证」 | 跳转登录 |
| 无权限 | 403 | 系统段「无权限」 | 提示 |
| 业务失败 / 校验失败 | 200 或 400 | `2xxxxx` 业务 / `3xxxx` 校验 | 展示 `msg` |
...
- **Feign：** 接口与 Fallback 优先在 `{domain}-api`；必须 Fallback；超时建议连接 3s、读取 10s。
...
| 分段 | 含义 | 多系统约定 |
|------|------|------------|
| `1xxxx` | 系统异常 | 语义由各 `{system}-common` **对齐维护同一套码值**（未认证、无权限、限流、系统异常等） |
| `2xxxxx`（6 位） | 业务异常 | 格式 `2` + 系统号（2 位）+ 业务序号（3 位），如 `201001`；系统号在 `docs/error-code-ranges.md` 登记（脚手架创建）；禁止跨系统同码不同义 |
| `3xxxx` | 参数校验 | 由 `common` + 全局异常统一；字段细节放 `data` |

- 错误码与文案用枚举/常量维护，禁止硬编码魔法字符串。
- 前端只按 `code` 分流，不解析号段。
...
- 主键 `id`：`Long` / `BIGINT`；策略 **yyMMdd(6) + 序列号(8) = 14 位**（例：`26092100000001`）。
- 序列号由序列表按**业务日**（`Asia/Shanghai`）递增；禁止数据库自增。
- 发号在 `framework`，多实例并发安全（行锁/唯一约束/按日重置）；禁止业务雪花/UUID 主键（对象存储文件名可用 UUID）。
- 两位年设计寿命至 2099；若需跨世纪再开变更，不在业务侧热修。
...
- Token：JWT，Access 30 分钟，Refresh 7 天；网关 + 服务双重鉴权；实现库见 2.1（选定前禁止业务自写解析）。
- 登录失败限制；敏感接口二次验证；关键接口限流见 6.11；CORS 仅信任域名（配置在网关，具体域名在 `docs/test-env.md` 或变更中登记）。
...
- `framework` 提供 `@Idempotent`（AOP）：Redis `SET NX`（或等价）占位成功才执行业务。
- TTL 默认 24h，可由注解参数覆盖（参数名由 `framework` 固定后写入变更）。
- 重复请求：返回**首次 Result**（占位时写入 Redis）或约定系统错误「重复请求」；禁止裸抛异常。
...
- 指数退避最多 3 次（1s/2s/4s）；超时与 5xx 可重试，4xx 不重试；必须打日志。
...
| 引入 Hutool / EasyExcel / POI / Quartz / XXL-Job / Druid / Sleuth / Spring Boot Admin | 见 2.2 |
| 自建 JWT 解析、自建 RedisTemplate | 统一 `framework` |
| 错误码跨系统撞号 | 按 6.4.2 登记系统号 |
