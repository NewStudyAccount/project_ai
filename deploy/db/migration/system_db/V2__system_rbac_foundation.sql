-- V2__system_rbac_foundation.sql
-- 库：各业务库 {system}_db（每系统独立执行，互不共享）
-- 约束：无物理外键；id BIGINT 禁自增；关联表物理删；无 password_hash
-- 变更：add-foundation-db-schema / 对照 docs/database-design.md §4
-- 关联：sys_user_role.user_id / sys_user_ref.user_id → auth_db.sys_user.id（跨库逻辑）

-- ----------------------------
-- 1. sys_sequence 本系统发号
-- ----------------------------
...
CREATE TABLE IF NOT EXISTS `sys_role` (
  ...
  `data_scope`  TINYINT      NOT NULL DEFAULT 1  COMMENT '数据范围预留 1 全部/2 本部门/3 本及下/4 本人/5 自定义',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='系统角色（按 system_code 隔离）';

-- ----------------------------
-- 3. sys_permission 权限（1 menu / 2 button / 3 api）
-- ----------------------------
CREATE TABLE IF NOT EXISTS `sys_permission` (
  `id`              BIGINT       NOT NULL            COMMENT '主键 14 位，禁自增',
  `system_code`     VARCHAR(32)  NOT NULL            COMMENT '所属系统标识',
  `parent_id`       BIGINT       NOT NULL DEFAULT 0  COMMENT '父权限 id，根为 0',
  ...
  `status`          TINYINT      NOT NULL DEFAULT 1  COMMENT '1 启用 / 0 停用',
  `method`          VARCHAR(16)  NOT NULL DEFAULT '' COMMENT 'api 方法 GET/POST…',
  `api_path`        VARCHAR(255) NOT NULL DEFAULT '' COMMENT 'api 路径',
  `permission_str`  VARCHAR(128) NOT NULL DEFAULT '' COMMENT '与 @PreAuthorize 对齐，可等于 code',
  `remark`          VARCHAR(255) NOT NULL DEFAULT '' COMMENT '备注',
  `create_time`     DATETIME     NOT NULL            COMMENT '创建时间',
  `update_time`     DATETIME     NOT NULL            COMMENT '更新时间',
  `create_by`       BIGINT       NOT NULL DEFAULT 0  COMMENT '创建人',
...
-- ----------------------------
-- 4. sys_user_role 用户↔角色
-- 【物理删】删除关联用 DELETE，勿用逻辑删（避免 UK 冲突）
-- user_id 仅逻辑指向 auth_db.sys_user.id，禁止存 password
-- ----------------------------
CREATE TABLE IF NOT EXISTS `sys_user_role` (
  `id`          BIGINT       NOT NULL            COMMENT '主键 14 位，禁自增',
  `user_id`     BIGINT       NOT NULL            COMMENT '用户 id = auth sys_user.id（跨库逻辑）',
  `role_id`     BIGINT       NOT NULL            COMMENT '角色 id = sys_role.id',
  `system_code` VARCHAR(32)  NOT NULL            COMMENT '所属系统标识（与角色一致）',
  `create_time` DATETIME     NOT NULL            COMMENT '创建时间',
  `create_by`   BIGINT       NOT NULL DEFAULT 0  COMMENT '创建人',
  `deleted`     TINYINT      NOT NULL DEFAULT 0  COMMENT '保留字段；删除请物理删',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_sys_user_role_user_role` (`user_id`, `role_id`),
  KEY `idx_sys_user_role_user_id` (`user_id`),
  KEY `idx_sys_user_role_role_id` (`role_id`),
  KEY `idx_sys_user_role_system_code` (`system_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户-角色关联（物理删）';

-- ----------------------------
-- 5. sys_role_permission 角色↔权限
-- 【物理删】
-- ----------------------------
CREATE TABLE IF NOT EXISTS `sys_role_permission` (
  `id`            BIGINT      NOT NULL            COMMENT '主键 14 位，禁自增',
  `role_id`       BIGINT      NOT NULL            COMMENT '角色 id = sys_role.id',
  `permission_id` BIGINT      NOT NULL            COMMENT '权限 id = sys_permission.id',
  `system_code`   VARCHAR(32) NOT NULL            COMMENT '所属系统标识',
  `create_time`   DATETIME    NOT NULL            COMMENT '创建时间',
  `create_by`     BIGINT      NOT NULL DEFAULT 0  COMMENT '创建人',
  `deleted`       TINYINT     NOT NULL DEFAULT 0  COMMENT '保留字段；删除请物理删',
...
-- ----------------------------
-- 6. sys_user_ref 用户展示投影（只读，无凭证）
-- 可 TRUNCATE 后由认证中心重建；status 仅展示，不作放行
-- ----------------------------
CREATE TABLE IF NOT EXISTS `sys_user_ref` (
  `user_id`        BIGINT       NOT NULL            COMMENT '主键 = auth sys_user.id，禁自增',
  `username`       VARCHAR(64)  NOT NULL DEFAULT '' COMMENT '用户名（展示）',
  `real_name`      VARCHAR(64)  NOT NULL DEFAULT '' COMMENT '姓名（展示）',
  `status`         TINYINT      NOT NULL DEFAULT 1  COMMENT '仅展示，不参与鉴权放行',
